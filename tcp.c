...
messages *processReadTCP(viz *sender, ssize_t start_ix)
{
  ...
  for (ix = start_ix; ix < sender->next_av_ix; ix++)
  {
    ...
    if (sender->buffer[ix] == '\n'){
      ...
      new->message[ix - start_ix + 2] = '\0';
      if (ix + 1 != sender->next_av_ix)
        // começar a proxima a ler a partir do proximo indice
  ...
  if (!message_end)
  {
    strncpy(sender->buffer, sender->buffer + start_ix, ix - start_ix + 1);
