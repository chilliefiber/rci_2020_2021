...
char *readCommand(enum instr *instr_code, int *errcode)
{
    ...
    *errcode = NO_ERROR;
    if (size_input == EOF)
        fprintf(stderr, "sscanf error reading input: %s\n", strerror(errno));
    //  EOF é negativo segundo o standard, portanto vai ser abrangido por este if
    //  a manpage parece pouco clara no que toca a retornar EOF vs 0
    if(size_input<=0)
    { 
...
int isIP(char ip[])
{
    ...
    int errcode;
    if((errcode = inet_pton(AF_INET, ip, &addr_ip)) == 0)
    {
        return 0 ;
    }
    else if (errcode == -1)
